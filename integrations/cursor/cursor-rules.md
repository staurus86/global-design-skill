...
- Next.js 16: use "use cache" + cacheLife() on slow data fetches.
...
**Option D — `.cursor/rules/` file** (Cursor 0.43+, new format):
1. Create `.cursor/rules/` directory in your project root
2. Copy `global-design.mdc` from this directory into `.cursor/rules/`
3. Cursor automatically loads all `.mdc` rules from that folder
